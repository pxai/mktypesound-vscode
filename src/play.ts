...
function Play(opts){
  ...
  this.play = function(what, next){
    ...
    //console.log("About to play: ", this.player, " opts: ", what, " args" ,next[this.player])
    child_process.execFile(this.player, [what, ...next[this.player]], function(err, stdout, stderr){
      next(err);
    })
  }
